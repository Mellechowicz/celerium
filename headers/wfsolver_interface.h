...
// This file provides a high-level interface for WFSolver
// that solves radial Schroedinger equation for a
// user-provided radial poential.
// For exemplary uses consult WFSolver in tests directory.
...
// The template used to solve the radial Schroedinger equation.
// The class Potential must provide the following methods:
// 1) int get_mesh(std::vector<double> v);
//    that initializes thee vector v with the mesh of points 
//    (distances from the center of the cooridnate systems).
//    The distances should be given in angstroms.
// 2) int get_mesh(std::vector<double> v);
//    that initializes v with the values of the potential for mesh points
//    generated by method get_mesh.
//    The energies should be given in eV.
...
class WFSolver {
 ...
 public:
  ...
  // Returns logs from the last call of GetEigenstate method.
...
potential returned an empty vector.");
    ...
    int status = FindEigenstate(interp,
      ...
      if (status == 0) {
        ...
        Interpolator radial_wave_function(eigenstate.wave_function);
                for (double r : mesh) {
          wave_function.push_back(radial_wave_function(r));
        }      
        return 0;
      }
...
potential returned an empty vector.");
    ...
    if (status == 0) {
      energy = eigenstate.energy;
      wave_function.SetSamples(eigenstate.wave_function);   
      return 0;
    }
