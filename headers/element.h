...
template <class RadialPotential = gsl::Interpolator,
          class OrbitalClass = OrbitalClass<gsl::Interpolator>> 
class Element {
 public:
...
mesh may contain only non-negative numbers.");
    ...
    if (status == 0) {
      ...
      gsl::Interpolator interp_result(eigenstate.wave_function);
      std::vector<gsl::sample_struct> mesh_samples;
      
      for (auto r : result_mesh) 
        mesh_samples.push_back({r, interp_result(r)});

      gsl::Interpolator wave_function_original_mesh_interp(mesh_samples);
...
equation solver did not converge (adjust the solver settings).");
  ...
  void AddOrbitalClass(int n, int l,
    ...
    std::vector<gsl::sample_struct> samples = this->radial_potential.GetSamples();
