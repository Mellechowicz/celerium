...
struct simple{
 int operator()(const double x[2], double f[1]){
	 f[0] = M_1_PI * exp(-x[0]*x[0] - x[1]*x[1]);
	 return 0;
 }
};
...
*/
  // Callable object
  engine.parameters.maxeval = 300;
  std::vector<std::pair<double,double>> b6(6,std::make_pair(-10.0,10.0));
  std::vector<std::pair<double,double>> b2(2,std::make_pair(-10.0,10.0));
...
//  engine.divonne_result(U1s,b6,resU,errU,pU,steps);
  engine.divonne_result([&B](double* x, double *f){return B(x,f);},b2,resU,errU,pU,steps);
...
*/
  ...
  engine.parameters.nComp = 4;
  engine.parameters.maxeval = 9999999;
  engine.parameters.flatness = 1.01;
